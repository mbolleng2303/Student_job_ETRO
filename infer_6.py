...
from nets.X_ray_1024_node_classification.load_net import gnn_model  # import GNNs
...
def eval(model, device, data_loader, epoch, infer = False, infer5=False):
    model.eval()
    epoch_test_loss = 0
    epoch_test_acc = 0
    nb_data = 0
...
def infer(MODEL_NAME, DATASET_NAME, params, net_params, dirs, last_epoch):
    ...
    dataset = LoadData(DATASET_NAME,inference=False,inf5=True)
    ...
    try:
        ...
        patient = np.reshape(np.loadtxt('C:/Users/Surface/PycharmProjects/Student_job_ETRO/data/Xray_1024/patient5.csv',
                                           delimiter=",", dtype=float), (400,1024,4))

        age, sex, feature = np.array(ref_graph.ndata['age']),np.array(ref_graph.ndata['age']),np.array(ref_graph.ndata['feat'])
        link = np.zeros([len(age)])
        model = gnn_model(MODEL_NAME, net_params)
        ...
        for p in range(len(patient[:,0,0])):#len(patient[:,0,0])
            ...
            test_graph = dgl.add_nodes(test_graph, 1, {'feat': feat, 'age': a, 'sex': s, 'predicted': torch.tensor([1])})
            test_graph = dgl.add_edges(test_graph, torch.tensor([0]), torch.tensor([0]),
                                      {'feat': torch.tensor([1]).long()})
            test_label = torch.tensor([res])
            testset.graph_lists[0] = test_graph
            testset.label_lists[0] = test_label
            test_loader = DataLoader(testset, batch_size=params['batch_size'], shuffle=False, drop_last=drop_last,
                                      collate_fn=dataset.collate)


            score,lab = eval(model, device, test_loader, epoch=0, infer=False, infer5=False)
        ...
        AUC = accuracy(lst_score,lst_label)
...
def main():
    ...
    #version = "GraphSage_X_ray_1024_GPU0_02h29m49s_on_Mar_10_2022"
    #last_epoch = [64, 63, 43, 68]
